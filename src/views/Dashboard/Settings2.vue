...
const companyName = ref('BK Group')
const companyEmail = ref('careers@bk.rw')
const companyAddress = ref('Kigali, Rwanda')
...
async function saveProfile() {
  ...
  if (
    !companyEmail.value.includes('@') ||
    !companyEmail.value.includes('.')
  ) {
    profileError.value = 'Please enter a valid email.'
    return
  }
...
<template>
  <div>
    <h1 class="text-3xl font-bold mb-2 text-gray-800">Company Settings</h1>
    <p class="text-gray-500 mb-8 text-sm">Manage your company information and security preferences.</p>
      ...
      <div class="p-8 space-y-5">
        <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
          ...
          <div>
            ...
            <input v-model="companyEmail" type="email" placeholder="Enter company email"
              class="w-full border border-gray-200 rounded-xl px-4 py-3 text-sm focus:outline-none focus:ring-2 focus:ring-teal-500 focus:border-transparent transition" />
          </div>
        </div>
        <div>
